...
func init() {
	AddInitEventHandler(func(typeOf Event, value interface{}) (responseOf EventResponse) {
		if typeOf == REVEL_BEFORE_MODULES_LOADED {
			if err := RegisterTemplateLoader(GO_TEMPLATE,
				func(loader *TemplateLoader) (TemplateEngine, error) {
				...
				log.Printf("error registering template loader %+v", err)
			}
		}
...
func (engine *GoEngine) ConvertPath(path string) string {
...
func (engine *GoEngine) Handles(templateView *TemplateView) bool {
	return EngineHandles(engine, templateView)
}
...
func (engine *GoEngine) ParseAndAdd(baseTemplate *TemplateView) error {
	...
	if engine.splitDelims != nil && strings.Contains(baseTemplate.Location(), ViewsPath) {
